...
def _extract_supplement_fields(text: str) -> dict[str, str]:
    ...
    label_to_field = dict(required_labels)
    ...
    missing_labels = [label for label, field_name in required_labels if not values.get(field_name)]
    if missing_labels:
        names = ", ".join(missing_labels)
        raise ValueError(f"補足設定の項目が不足しています: {names}")
...
def load_setting_from_markdown(file_path: str = "persona.md") -> JournalSetting:
    ...
    role_text = _pick_section(sections, ["主人公"])
    background_text = _pick_section(sections, ["背景"])
    period_text = _pick_section(sections, ["期間"])
    weekly_theme = _pick_section(sections, ["1週間のテーマ"])
    weekday_style = _pick_section(sections, ["平日傾向"])
    weekend_style = _pick_section(sections, ["休日傾向"])
    tone_keywords = _pick_section(sections, ["文体"])
    target_reader = _pick_section(sections, ["想定読者"])
    realism_constraints = _pick_section(sections, ["現実味の制約"])
    supplement_text = _pick_section(sections, ["補足設定"])
    supplement = _extract_supplement_fields(supplement_text)
    event_text = sections.get("イベント", "")
